...
def write_processed_file(file, model, processed_files):
    with open(
            '{}/processed_files/{}_{}_processed_files.json'.format(output_dir, file.split('.')[0], model.split('.')[0]),
            'w') as f:
        f.write(json.dumps([]))
...
def write_classify_result(file, model, result):
    ...
    with open(
            '{}/classify_result/{}_{}_classify_result.json'.format(output_dir, file.split('.')[0], model.split('.')[0]),
            'w') as f:

...
def predict(process_id, filename, inference_sess, input_layer, output_layer):
    ...
    with tf.Session() as sess:
        ...
        t1 = time.time()
        type_class=1#除了unknow有几类
        ty_num=np.zeros([type_class]) #统计每类多少图片真实
        ty_num_predict=np.zeros([type_class])#统计每类多少图片预测
        ty_num_true=np.zeros([type_class])#统计每类多少图片预测正确
        try:
            while True:
                ...
                for i, pred in enumerate(predictions):
                    ...
                    filetype=0 #unknow类型
                    try:
                        _filename_spilt=str(_filepath[i], encoding='utf-8').split('_')
                    except:
                        _filename_spilt=str('None_none', encoding='utf-8').split('_')
                    
                    if _filename_spilt[0]=='id':
                        ty_num[0]+=1
                        filetype=1#id真实类型
                    ...
                    if predict_result == 'back_id_image': 
                            ty_num_predict[0]+=1 #id预测类型
                            #print(ty_num_predict[0])
                            #print(predict_result)
                    if predict_result == 'back_id_image'and filetype==1:
                         ty_num_true[0]+=1
            ...
            pre_all=ty_num_true/ty_num #查全率
            pre_true=ty_num_true/ty_num_predict #查准率
            #print(ty_num,ty_num_true,ty_num_predict,pre_all,pre_true)
            print("recall ratio:",pre_all)
            print("precision ratio",pre_true)
            t2 = time.time()
...
def write_reading_lock(file, model, processed_files):
    with open('{}/eval_reading_lock/{}'.format(output_dir, file), 'w') as f:
        f.write(json.dumps([]))
...
def check_reading_lock():
    ...
    processed_files = []
    if not tf.gfile.Exists("{}/eval_reading_lock".format(output_dir)):
        tf.gfile.MakeDirs("{}/eval_reading_lock".format(output_dir))
    for path, dir, files in os.walk('{}/eval_reading_lock'.format(output_dir)):
        for file in files:
